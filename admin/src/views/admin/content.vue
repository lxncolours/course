...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      listContentFiles() {
        let _this = this;
        _this.$ajax.get(process.env.VUE_APP_SERVER + '/business/admin/courseContentFile/list/' + _this.course.id).then((response)=>{
          let resp = response.data;
          if (resp.success) {
      ...
      afterUploadContentFile(response) {
        ...
        file.url = file.path;
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/admin/courseContentFile/save', file).then((response)=>{
          let resp = response.data;
          if (resp.success) {
      ...
      delFile(f) {
        ...
        Confirm.show("删除课程后不可恢复，确认删除？", function () {
          _this.$ajax.delete(process.env.VUE_APP_SERVER + '/business/admin/courseContentFile/delete/' + f.id).then((response)=>{
            let resp = response.data;
            if (resp.success) {
